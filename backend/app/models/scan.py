...
class Scan(Base):
    __tablename__ = "scans"
    __table_args__ = (
        UniqueConstraint("user_id", "local_id", name="uq_scans_user_local_id"),
    )
    ...
    pathology_id: Mapped[int | None] = mapped_column(
        ForeignKey("pathologies.id"), nullable=True
    )
    commune_id: Mapped[int | None] = mapped_column(
        ForeignKey("communes.id"), nullable=True
    )
    ...
    hors_ligne: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
